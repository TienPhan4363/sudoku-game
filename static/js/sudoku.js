const newGrid = size => {
    ...
    for (let i = 0; i < Math.pow(size, 2); i++) {
        arr[Math.floor(i / size)][i % size] = CONSTANT.UNASIGNED;
    }
...
const isSafe = (grid, row, col, value) => {
    return (
        ...
        value !== CONSTANT.UNASIGNED
    );
};

// find UNASIGNED cell
const findUnasignedPos = (grid, pos) => {
    for (let row = 0; row < CONSTANT.GRID_SIZE; row++) {
        for (let col = 0; col < CONSTANT.GRID_SIZE; col++) {
            if (grid[row][col] === CONSTANT.UNASIGNED) {
                pos.row = row;
                pos.col = col;
                return true;
            }
...
const isFullGrid = grid => {
    return grid.every((row, i) => {
        return row.every((value, j) => {
            return value !== CONSTANT.UNASIGNED;
        });
    });
};
...
const sudokuCreate = grid => {
    ...
    if (!findUnasignedPos(grid, unasigned_pos)) return true;

    let number_list = shuffleArray([...CONSTANT.NUMBERS]);

    let row = unasigned_pos.row;
    let col = unasigned_pos.col;
    ...
    number_list.forEach((num, i) => {
        if (isSafe(grid, row, col, num)) {
            ...
            grid[row][col] = CONSTANT.UNASIGNED;
        }
    });
...
const removeCells = (grid, level) => {
    ...
    while (attemps > 0) {
        ...
        res[row][col] = CONSTANT.UNASIGNED;
        attemps--;
    }
    return res;
};
